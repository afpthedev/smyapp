...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Query(
        value = "select notification from Notification notification left join fetch notification.appointment left join fetch notification.recipient",
    ...
    @Query(
        "select notification from Notification notification left join fetch notification.appointment left join fetch notification.recipient"
    )
    List<Notification> findAllWithToOneRelationships();

    @Query(
        "select notification from Notification notification left join fetch notification.appointment left join fetch notification.recipient where notification.id =:id"
    )
