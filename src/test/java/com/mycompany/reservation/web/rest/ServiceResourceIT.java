...
class ServiceResourceIT {
    ...
    public static Service createEntity() {
        return new Service().name(DEFAULT_NAME).description(DEFAULT_DESCRIPTION).duration(DEFAULT_DURATION).price(DEFAULT_PRICE);
    }
    ...
    public static Service createUpdatedEntity() {
        return new Service().name(UPDATED_NAME).description(UPDATED_DESCRIPTION).duration(UPDATED_DURATION).price(UPDATED_PRICE);
    }
